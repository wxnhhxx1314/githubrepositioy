...
and scattering effects also bring challenges in the design of miniatured on-body antennas. %What challanges?
Characteristics of antennas and on-body channels have been widely studied \cite{1}-\cite{4}. In advance, textile patch antennas has also been
...
%Motivation: 
%High polarization selectivity of on-body channels calls for on-body antenna optimization in order to gain the channel polarization gain or polarization diversity.
%To further lower the powre comsumption, or to alleviate the channel fading effect on dynamic  bodies.
%Two options: 1. design antennas with optimized polarization: if the antenna's multi-polarization, how to design the antenna to ensure every polarization component can achieve better channel, and to invstiagate the antenna's radiation gain, 2. antenna placement optimization
%Why linearly polarized antennas? before analyzing any complex polarization antennas, we must understand the basic polarization performance via linearly polarized antennas
%Why analyze in this way? This paragraph:
...
%Related work: (in a section or in 2 paragraphs)
%1. antenna design in on-body communications
%2. polarized antenna design in on-body communications
%3. on-body channel polarization 
%4. on-body antenna performance analysis, antenna optimization 

%What have we done: (a complete summary of your work)
...
linearly polarized antennas. Section \ref{sec:body_model} presents the layered human chest model. Section \ref{sec:analysis} analyzes the
...
the body as shown in Fig. \ref{fig:3}. %Explain the horizontal, vertical polarization, angles, E-plane, H-plane
\begin{table}[!htb]
\centering
\begin{tabular}{|c|c|c|}
\hline
...
The focus of this paper is to observe the antenna performance after loading human by changing the antenna placement.  The distance between the antenna and human body surface, denoted $d$, is an important factor of antenna performance. We summarize the variation of the return loss, denoted as $S_{11}$, and the bandwidth, denoted as $B$, in the range of 0.5cm$<$d$<$6cm, and compare the changes of antenna gain pattern of different $d$.
% We compare the antenna polarization performance on E-Plane and H-Plane under Vertical & horizontal polarization as well, based on what parameters?

%Uniformed subsection structure

\subsection{IFA}

Fig. \ref{fig:5} shows the changes of antenna matching performance and bandwidth upon the $d$ variation. For horizontal polarization %as depicted in Fig. \ref{},
0.4 GHz$<$B$<$0.52 GHz, when $d$= 2.5 cm, $B$ reaches the maximum.  

%Why using approximation?
The bandwidth of vertical polarization %depicted in Fig. \ref{},
of IFA is described as Eq. \ref{eq:eps_1}.
...
%Which polarization?  
Under both polarizations, the smalledst bandwidth is observed at $d$=0.5cm. %Why this happens? (Body coupling effect)
%, the bandwidth is the narrowest, and the antenna is not suitable for placing on body surface this time.
...
\label{fig:5b}
\end{subfigure}
\caption{IFA: Comparison of S11 and $B$ of horizontal polarization and vertical polarization with different difference
$d$ }
\label{fig:5}
\end{figure}
...
In vertical polarziation, $S_{11}$ shows unimdal distribution for 0.5 cm$<$d$<$4 cmm, where its minimal value is observed at $d= 1.5 cm$.

In ranges 0.5cm$<$d$<$1.2cm and $d$$>$2cm, the horizontal polarization of S11 is always smaller than vertical polarization. It indicates that within these ranges, the IFA antennas in horizontal polarization will achieve better performance than in vertical polarization.
...
%1. insensitivity of d to the radiation pattern of antennas in both polarizations

%2. under d=5cm and d=1.5cm, the radiation pattern analysis for both polarizations
...
For H-plane pattern, when $0\,^{\circ}$$<$ $\varphi$$<$$30\,^{\circ}$, $310\,^{\circ}$$<$ $\varphi$$<$$330\,^{\circ}$,
$S^{2}$$>$5, indicating that the difference between the gain in the same direction is great and antenna directional
gain is sensitive to distance. 

The directional gain distribution of vertical polarization at different distances is
uniform and not sensitive to the $d$. By comparing the antenna polarization performance at different distances, it¡¯s found that the impact on antenna is not
significant for $d$ variation.  We then select $d = 5 cm$ and $d = 1.5 cm$ for horizontal polarization and vertical polarization,
respectively.

E-plane radiation pattern of horizontal polarization and vertical polarization after loading body model
...
For horizontal polarization, the bandwidth approximated by Eq. \ref{eq:eps_3}.
...
maximum. The fitting equation for S11 of vertical polarization is given in Eq. \ref{eq:eps_4}
...
%Replace When .... into the following sentence:
%When $d$=5.5cm, S11 takes the minimum. 
The minimal $S_{11}$ of the MIFA antennas is observed at $d$= 5.5 cm.
When 1cm$<$$d$$<$2.5cm, S11 of vertically polarized is always smaller than
horizontal polarization, and the bandwidth is always wider than horizontal polarization. In this range, the antenna is
more suitable for placing on the surface of the human body in vertically polarized. When $d$=0.5cm, S11 of vertical
polarization take the maximum and the bandwidth is the narrowest, so the antenna is not suitable for placing on human
body surface. The S11 curve of horizontal polarization is flat and when $d$=4cm, $B$ reaches the maximum, so the
antenna is most suitable to place on human body surface.


In horizontal polarization, the standard deviation (STD) of the radiatin gain are uniformly distributed as shown in Fig. \ref{}, showing insensitivity to $d$.

For E-plane pattern of vertical polarization, when $30\,^{\circ}$$<$$\varphi$$<$$150\,^{\circ}$,
...
For H-plane gain pattern of the horizontal polarization, the gain curve is always smooth and the body has negliable effect
...
surface in vertical polarization, the antenna performance is the best. We can see form the S11 curve, the trend of
...
S11 of horizontal polarization is always greater than vertical polarization. When 0cm$<$$d$$<$3cm, the bandwidth of
...
%general conclusion: after the above analysis, what can we draw?
%1. common conclusion from the 3 types of antennas
%2. different conslusions from each antenna? what's is the performance difference of them in different polarizations?
